...
# DB tablolarını otomatik oluştur (uygulama açılışında)
...
async def cmd_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Liderlik tablosunu (Top 200) Telegram'da gösterir"""
...
async def post_score(request: Request):
    ...
    try:
        ...
        score = int(body["score"])              # Bu “run” skoru; DB’de max ile karşılaştıracağız
    ...
    async with SessionLocal() as session:
        ...
        if not obj:
            ...
            new_score = max(obj.score or 0, score)   # <- max politika
            obj.username = username or obj.username
            obj.score = new_score

        await session.commit()

    # Telegram Game API — setGameScore toplam/maks bekler
    await tg_app.bot.set_game_score(
        user_id=user_id,
        score=new_score,                 # delta değil, toplam/maks
        chat_id=chat_id,
