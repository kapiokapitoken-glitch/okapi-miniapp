...
# DB engine (gerekirse SSL doğrulamayı açarsın)
...
# Uygulama açılırken: tabloları kur + Telegram botu başlat
...
# Uygulama kapanırken: Telegram botu durdur
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cq and cq.game_short_name == GAME_SHORT_NAME:
        ...
        # Web app'e taşıyacağımız imzalı payload
...
async def cmd_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Top 200 liderlik tablosunu Telegram’da gösterir"""
...
# Tarayıcıyla GET yapılırsa 405 yerine açıklama dönsün (opsiyonel)
@app.get("/bot/webhook")
def webhook_get_info():
    return PlainTextResponse("This endpoint accepts POST updates from Telegram.")

# --- API: Score (MAX skor politikası) ---
@app.post("/api/score")
async def post_score(request: Request):
    body = await request.json()
    try:
        user_id    = int(body["user_id"])
        score      = int(body["score"])         # run skoru; DB’de max ile karşılaştıracağız
    ...
    # DB güncelle (max skor tut)
    async with SessionLocal() as session:
        existing = await session.execute(select(Score).where(Score.user_id == user_id))
        obj = existing.scalar_one_or_none()

        if not obj:
            new_score = max(0, score)
            obj = Score(user_id=user_id, username=username, score=new_score)
            session.add(obj)
        else:
            new_score = max(obj.score or 0, score)
            obj.username = username or obj.username
            obj.score = new_score

        await session.commit()

    # Telegram Game API skoru (toplam/maks gönderilir)
